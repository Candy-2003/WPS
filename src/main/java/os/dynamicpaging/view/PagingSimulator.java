...
public class PagingSimulator extends JFrame {
    ...
    private int allocatedBlocks = 4;                   // 分配的内存块数（默认4）
    private int blockSizeKB = 1;                       // 内存块大小（KB）
    ...
    private void initSystem() {
        ...
        for (int i = 0; i < 64; i++) {
            ...
            currentDiskLoc += 1 + rand.nextInt(20); // 增加1-20随机数
        }
    ...
    private void executeInstruction() {
        try {
            ...
            if (entry.present) {
                physicalAddr = entry.frame * 1024 + offset; // 固定块大小1KB
                result = "不缺页";
            } else {
