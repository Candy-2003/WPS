...
public class PagingSimulator extends JFrame {
    ...
    private void initComponents() {
        ...
        // 历史记录
        historyModel = new DefaultTableModel(new Object[]{"时间", "类型", "页号", "偏移", "结果"}, 0);
    ...
    private void executeInstruction() {
        try {
            ...
            String operationType = isWrite ? "写入" : "读取";

            // 记录操作
            Object[] historyEntry = new Object[5];
            historyEntry[0] = now;
            historyEntry[1] = operationType;
            historyEntry[2] = page;
            historyEntry[3] = offset;

            if (page < 0 || page >= 64 || offset < 0 || offset >= 1024) {
                historyEntry[4] = "无效地址";
                historyModel.addRow(historyEntry);
            ...
            logArea.append(String.format("地址转换: %d页+%d → 物理地址%d (块%d)\n",
    ...
    private void handlePageFault(int page) {
        logArea.append(">>> 缺页处理: 页" + page + "\n");
        addHistoryRecord("系统", "缺页处理", page, -1, "开始处理");

        int freeFrame = findFreeFrame();
        if (freeFrame != -1) {
            loadPage(page, freeFrame);
            addHistoryRecord("系统", "页面加载", page, freeFrame, "空闲块");
            return;
        }

        int victimFrame = findVictimFrame();
        replacePage(victimFrame, page);
        addHistoryRecord("系统", "页面置换", page, victimFrame, "淘汰页" + physicalMemory[victimFrame].page);
    }

    private void addHistoryRecord(String type, String operation, int page, int frame, String result) {
